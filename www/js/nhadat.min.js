...
a.path("/login.html"),e.show({type:"error",msg:f.data.flash}));return d.reject(f)};return function(a){return a.then(f,g)}}])}]).config(["$compileProvider",function(a){a.aHrefSanitizationWhitelist(/^\s*(https?|mailto|tel|sms|file):/)}]).run(["$rootScope","$location","Auth","Flash",function(a,b,d,c){var e=["/diadanh.html","/sale.html","/share.html","/update.html","/account.html"],f=["/diadanh.html","/sale.html"];a.isPath="";a.$on("$routeChangeStart",function(g,h,k){a.isLogin=d.isLoggedIn();a.User=d.user();
